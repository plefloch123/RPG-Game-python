...
class Player(Entity):
	def __init__(self,pos,groups, obstacle_sprites, create_attack, destroy_attack, create_magic):
		...
		self.hitbox = self.rect.inflate(-6,HIBOX_OFFSET['player']) # make the hitbox smaller than the image
	...
	def input(self):
		...
		if not self.attacking:
			...
			if keys[pygame.K_SPACE]:
				self.attacking = True
				self.attack_time = pygame.time.get_ticks() # only run once
				self.create_attack()
	...
	def cooldowns(self):
		current_time = pygame.time.get_ticks() # run multiple times
	...
	def animate(self):
		...
		# flicker
		if not self.vulnarable:
            # flickering (function that togles between 0 and 255)
